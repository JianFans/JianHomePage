...
实施说明：当前环境没有可调用的内置 imagegen 工具，且未获授权使用需要 `OPENAI_API_KEY` 的 CLI 路径。首版改用固定 seed 的本地程序化 WebP fixture，并在资源版权元数据中明确标注；正式授权素材可沿用相同资源 ID 和路径替换。

- [x] **步骤 2：为首页用途裁切和压缩资源**

英雄图保持至少 `1600 × 1000`，封面为正方形，影像海报为 `16:9`，片段使用 `4:5`、`1:1` 和 `3:4`。所有资源转换为 WebP，单张文件大小和像素尺寸写入资源清单。

- [x] **步骤 3：生成 3 秒无版权提示音样本**
